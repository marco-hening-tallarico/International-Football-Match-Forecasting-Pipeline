# 26_model_draw_aware_features.R
#
# Model 26: compares draw-aware Elo feature variants against the Model 24 safe
# set, using the same splits and model families. Variant choice is by validation
# log loss; test split is for reporting only.
#
# Reads: data/processed/international_modeling_table.csv
#
# Writes: reports/tables/model_26_* and reports/figures/model_26_*
#
# Notes:
# - Variants: baseline_safe, draw_abs_only, draw_smooth, draw_smooth_neutral.
...
# 4. Base safe features and forbidden predictors
...
derived_draw_features <- c(
    "abs_rating_diff",
    "rating_diff_squared",
    "neutral_numeric",
    "neutral_x_abs_rating_diff"
)
...
missing_base_features <- setdiff(base_safe_features, names(modeling))

if (length(missing_base_features) > 0) {
    stop(
        "Modeling table is missing base safe feature columns: ",
        paste(missing_base_features, collapse = ", "),
        call. = FALSE
    )
}
...
feature_variants <- list(
    ...
    draw_abs_only = c(base_safe_features, "abs_rating_diff"),
    draw_smooth = c(base_safe_features, "abs_rating_diff", "rating_diff_squared"),
    draw_smooth_neutral = c(
        base_safe_features,
        "abs_rating_diff",
        "rating_diff_squared",
        "neutral_x_abs_rating_diff"
...
df <- df |>
    dplyr::select(
        ...
        data_split,
        match_result,
        dplyr::all_of(base_safe_features)
    ) |>
    tidyr::drop_na()
...
readr::write_csv(filter_counts, "reports/tables/model_26_filter_counts.csv")

# 6. Derived draw-aware features (after filtering)

df <- df |>
    dplyr::mutate(
        date = as.Date(date),
        match_result = factor(match_result, levels = c("H", "D", "A")),
        dplyr::across(dplyr::all_of(base_safe_features), ~ {
            if (is.logical(.x)) {
                as.integer(.x)
            } else {
                .x
        ...
        rating_diff_squared = rating_diff^2,
        neutral_numeric = as.integer(neutral),
...
readr::write_csv(split_summary, "reports/tables/model_26_split_summary.csv")
readr::write_csv(feature_variants_tbl, "reports/tables/model_26_feature_variants.csv")

# 7. Variant loop: correlations, model fitting, metrics
...
for (variant_name in names(feature_variants)) {
    ...
    missing_variant_features <- setdiff(selected_features, names(df))

    if (length(missing_variant_features) > 0) {
        stop(
            "Missing features for variant ",
            variant_name,
            ": ",
            paste(missing_variant_features, collapse = ", "),
            call. = FALSE
        )
    }
...
readr::write_csv(all_metrics, "reports/tables/model_26_metrics.csv")
readr::write_csv(all_classwise, "reports/tables/model_26_classwise_metrics.csv")
readr::write_csv(all_confusions, "reports/tables/model_26_confusion_matrices.csv")
readr::write_csv(all_predictions, "reports/tables/model_26_predictions.csv")
readr::write_csv(all_correlations, "reports/tables/model_26_high_feature_correlations.csv")

# Note: model_26_high_feature_correlations.csv contains ALL pairwise correlations;
# high_correlation column flags |r| >= 0.90.

# 8. Best validation models
...
readr::write_csv(
    ...
    "reports/tables/model_26_best_validation_models.csv"
)

# 9. Comparison to Model 24
...
if (file.exists(model_24_metrics_path)) {
    ...
    model_24_aligned <- model_24_metrics |>
        dplyr::mutate(
            feature_variant = "model_24_baseline_safe",
            source = "model_24"
        )

    model_26_for_compare <- all_metrics |>
        dplyr::mutate(source = "model_26")

    compare_metrics <- dplyr::bind_rows(model_24_aligned, model_26_for_compare) |>
        dplyr::select(
            source,
            feature_variant,
            model,
    ...
    readr::write_csv(
        compare_metrics,
        "reports/tables/model_26_compare_to_model_24.csv"
    )
} else {
    message("model_24_metrics.csv not found; skipping comparison table.")
}
...
validation_log_loss_plot <- all_metrics |>
    ...
    ggplot2::labs(
        title = "Model 26 validation log loss by feature variant",
        x = "Feature variant",
...
ggplot2::ggsave(
    filename = "reports/figures/model_26_validation_log_loss_by_variant.png",
...
test_log_loss_plot <- all_metrics |>
    ...
    ggplot2::geom_col(position = "dodge") +
    ggplot2::labs(
        title = "Model 26 test log loss by feature variant",
        x = "Feature variant",
        y = "Multiclass log loss",
        fill = "Model"
    ) +
...
ggplot2::ggsave(
    filename = "reports/figures/model_26_test_log_loss_by_variant.png",
...
draw_recall_plot <- all_metrics |>
    ...
    ggplot2::geom_col(position = "dodge") +
    ggplot2::labs(
        title = "Model 26 test draw recall by feature variant",
        x = "Feature variant",
        y = "Draw recall",
        fill = "Model"
    ) +
...
ggplot2::ggsave(
    filename = "reports/figures/model_26_draw_recall_by_variant.png",
...
predicted_vs_actual_draw_plot <- all_metrics |>
    ...
    ggplot2::labs(
        title = "Model 26 predicted vs actual draw rate (test set)",
        x = "Feature variant",
        y = "Rate",
        fill = NULL
    ) +
...
ggplot2::ggsave(
    filename = "reports/figures/model_26_predicted_vs_actual_draw_rate.png",
...
macro_f1_plot <- all_metrics |>
    ...
    ggplot2::geom_col(position = "dodge") +
    ggplot2::labs(
        title = "Model 26 test macro F1 by feature variant",
        x = "Feature variant",
        y = "Macro F1",
        fill = "Model"
    ) +
...
ggplot2::ggsave(
    filename = "reports/figures/model_26_macro_f1_by_variant.png",
    plot = macro_f1_plot,
    width = 10,
    height = 6,
    dpi = 300
...
draw_aware_variants <- setdiff(names(feature_variants), "baseline_safe")

draw_aware_beat_baseline <- validation_ranking |>
    dplyr::filter(
        feature_variant %in% draw_aware_variants,
        log_loss < best_baseline_log_loss
    )
...
draw_recall_improvements <- all_metrics |>
    ...
    dplyr::filter(feature_variant %in% draw_aware_variants) |>
    dplyr::mutate(
...
message("")
message("8. Draw-aware variants beating baseline_safe on validation log loss:")
if (nrow(draw_aware_beat_baseline) == 0) {
    message("  None.")
} else {
    print(draw_aware_beat_baseline)
}
message("")
message("9. Draw-aware variants with improved draw recall without materially")
message("   worsening validation log loss (threshold = ", LOG_LOSS_MATERIAL_THRESHOLD, "):")
if (nrow(worth_adopting) == 0) {
    message("  None.")
} else {
    print(worth_adopting)
}
message("")
